...
def format_documents(documents: list) -> dict:
    ...
    # Import necessary functions - using code from main script
    # For brevity, I'm assuming these functions are imported from the main script

    for i, url in enumerate(documents):
        logger.info(f"Processing practice {i+1}/{len(documents)}: {url}")

        # Call process_practice function from main script
        # This would process the URL and save to output_dir
...
def ingest_documents(input_artifact: str) -> None:
    ...
    # Call ingest_to_elasticsearch function from main script
...
def main():
    ...
    # Check if we're running in pipeline mode or standalone mode
    pipeline_mode = os.environ.get("KUBEFLOW_ENDPOINT") is not None
